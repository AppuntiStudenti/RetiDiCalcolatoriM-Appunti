...
generale, la procedura di ripristino consiste infatti prima nell'identificazione del tipo di guasto, e in base a ciò si
...
\begin{itemize}
 ...
 invariante, e questo viene identicato dagli  altri processori
 \item \textit{Fail-safe}: come il precedente, tranne che gli altri processori
 non se ne accorgono: in questo caso quindi non è identicato, ma potrebbe
...
blocca. Le reti poi possono aggiungere problemi (se un router per esempio non risponde più, si potrebbe avere che la
...
\begin{enumerate}
 ...
 efficienti per identicarli correttamente: si vuole un \textit{metalivello
 efficiente}!
\end{enumerate}
...
(identificazione e recupero dal gestore o da chi altro: dovrà essere deciso in fase progettuale).
